...
**Package Organization**: The BaseNode class SHALL be provided by the `@workflow/core` package. Built-in node implementations SHALL be provided by the `@workflow/nodes` package, which depends on `@workflow/core`, `@workflow/interfaces`, and `@workflow/schemas`.
